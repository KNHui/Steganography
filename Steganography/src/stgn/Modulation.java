...
public class Modulation extends JFrame {
	...
	Modulation() {
				...
				try {
					...
					inputOuter: for (int h = 0; h < biHeight; h++) {
						for (int w = 0; w < biWidth; w++) {
							...
							if ((red == 255 || red == 254) && (green == 255 || green == 254)
									&& (blue == 255 || blue == 254)) {
								amountWhite++;
					...
					infoOuter: for (int h = endPos[0]; h >= 0; h--) {
						for (int w = endPos[1] - wCount; w >= 0; w--) {
							...
							img.setRGB(w, h, rgb);
							wCount++;
							if (index == binaryInfo_array.length && j == 2 && last == false) {
		...
		la6.setSize(200, 20);
		la6.setLocation(400, 75);

		decimagefile.setSize(110, 20); // 복호화할 이미지
		decimagefile.setLocation(570, 75);

		JButton bt2 = new JButton("복호화");
		bt2.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) { // 복호화 코드
				// TODO Auto-generated method stub
